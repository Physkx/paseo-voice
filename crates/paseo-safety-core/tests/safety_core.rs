...
#[test]
fn a_seeded_queue_keeps_dedup_history_but_carries_no_proposal_or_active_context() {
    // A prior connection reads and responds to a reply, then hands off its queue.
    let mut previous = SafetyCore::new(100);
    let handled = summary_id("summary-handled");
    let handled_thread = thread_id("thread-handled");
    observe_ready_and_activate(&mut previous, &handled, &handled_thread, "reply-handled");
    propose(
        &mut previous,
        &handled,
        &proposal_id("proposal-handled"),
        1,
        1,
    );
    previous
        .apply(Command::ConfirmResponse {
            proposal_id: proposal_id("proposal-handled"),
            interaction: InteractionSequence::new(2),
            now_ms: 2,
        })
        .expect("confirm handoff response");
    let snapshot = previous.queue_snapshot();

    // A fresh connection seeds from that snapshot.
    let mut next = SafetyCore::new(100);
    next.seed_queue(snapshot);

    // The already-handled reply stays deduplicated across the reconnect.
    assert_eq!(
        next.apply(Command::ObserveReply {
            summary_id: summary_id("summary-handled-again"),
            source_thread_id: handled_thread,
            source_reply_id: reply_id("reply-handled"),
            observed_at_ms: 10,
        }),
        Ok(Applied::DuplicateReply)
    );

    // No active context or proposal is inherited: a fresh read works normally
    // and a response can be proposed and confirmed against the new reply.
    let fresh = summary_id("summary-fresh");
    observe_ready_and_activate(&mut next, &fresh, &thread_id("thread-fresh"), "reply-fresh");
    propose(&mut next, &fresh, &proposal_id("proposal-fresh"), 3, 20);
    assert!(matches!(
        next.apply(Command::ConfirmResponse {
            proposal_id: proposal_id("proposal-fresh"),
            interaction: InteractionSequence::new(4),
            now_ms: 21,
        }),
        Ok(Applied::DispatchAuthorized(_))
    ));
