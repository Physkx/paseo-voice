...
| Voice         | OpenAI Realtime, xAI Grok Voice, or custom endpoint; mock when cloud creds missing    |
| Summaries     | Manual reads plus opt-in alpha polling announcements through Realtime                 |
| Cleanup model | OpenAI-compatible chat completions; official xAI (`https://api.x.ai/v1`) supported    |
| Secrets       | Bitwarden, 1Password (OpenAI + Paseo + optional model key), env, Grok OAuth store     |
