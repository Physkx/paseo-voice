...
| D013 | Reply identity       | Assign a broker observation ID from thread ID, exact output digest, and completion generation     | Supplies stable internal provenance because Paseo 0.1.107 logs expose no reply ID           | Require an upstream reply ID before enabling automated observation |
